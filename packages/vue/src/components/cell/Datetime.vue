<script setup lang="ts">
    ...
    import { normalizeDatetime, formatDatetime } from '../DatetimeHelpers.js'


    const props = defineProps<FrontoProps<'Datetime'>>()

    const resolvedValue = useFrontoValue(props, normalizeDatetime)
    const valueLabel = computed(() => formatDatetime(resolvedValue.value as string))

    const enumInvalid = computed(() => {
        return !isValueInEnum<'Datetime'>(resolvedValue.value as string, props.enum)
    })
