...
<template>
    <div v-if="isVisible" class="flex flex-col gap-1.5" :class="{ 'opacity-60': isDisabled }">
        <div class="tooltip tooltip-top z-50" :data-tip="props.description">
            <span class="text-xs tracking-wide cursor-default">
                <slot :disabled="isDisabled" :readable="isReadable" :writable="isWritable" />
            </span>
        </div>
        <small v-if="errorMessage" class="text-xs text-error">{{ errorMessage }}</small>
    </div>
</template>
