...
import { normalizeDatetime, parseDatetimeInput, toDatetimeLocalInput } from '../DatetimeHelpers'
...
const inputValue = ref(toDatetimeLocalInput(resolvedValue.value))
...
async function handleChange(event: Event): Promise<void> {
    ...
    const nextValue = parseDatetimeInput(rawValue)

    if (nextValue === undefined) {
        uiError.value = 'Value must be a valid datetime.'
        return
    }
    if (!isValueInEnum<'Datetime'>(nextValue, props.enum)) {
        uiError.value = 'Value must be one of enum values.'
        return
    }
    uiError.value = ''
    inputValue.value = toDatetimeLocalInput(nextValue)
