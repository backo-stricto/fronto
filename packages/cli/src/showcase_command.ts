...
function do_showcase(frontoComponentsPath: string, projectRootPath: string): void {
    ...
    tui.finishLine(
        `${tui.commandInfo('SHOWCASE')} ${tui.info()} Fronto components path [${pc.inverse(frontoComponentsPath)}]`,
    )
    tui.finishLine(
        `${tui.commandInfo('SHOWCASE')} ${tui.info()} Project root [${pc.inverse(projectRootPath)}]`,
    )
    // Check if the project root path exists
    if (!fileSystem.existsSync(projectRootPath)) {
        tui.finishLine(
            `${tui.commandInfo('SHOWCASE')} ${tui.error()} Error: Project root path does not exist: ${pc.inverse(projectRootPath)}`,
        )
        exit(1)
    }
    ...
    if (!fileSystem.existsSync(frontoComponentsPath)) {
        tui.finishLine(
            `${tui.commandInfo('SHOWCASE')} ${tui.error()} Error: Fronto components path does not exist: ${pc.inverse(frontoComponentsPath)}`,
        )
        exit(1)
    }
    ...
    if (fileSystem.existsSync(showcaseEntryPointPath)) {
        tui.finishLine(
            `${tui.commandInfo('SHOWCASE')} ${tui.info()} Deleting existing showcase entry point at ${pc.inverse(showcaseEntryPointPath)}...`,
        )
        fileSystem.rmSync(showcaseEntryPointPath, { recursive: true, force: true })
    }
    // Create the showcase entry point
    tui.finishLine(
        `${tui.commandInfo('SHOWCASE')} ${tui.info()} Creating showcase entry point at ${pc.inverse(showcaseEntryPointPath)}...`,
    )
    ...
    const frontoVuePackagePath: string = path.dirname(
        require.resolve('@backo-stricto/fronto-vue/package.json'),
    )
    ...
    tui.finishLine(
        `${tui.commandInfo('SHOWCASE')} ${tui.info()} Copying showcase assets: ${pc.inverse(showcaseAssetsPath)} \u279c ${pc.inverse(showcaseDirPath)}...`,
    )
    fileSystem.cpSync(showcaseAssetsPath, showcaseDirPath, { recursive: true })
    tui.finishLine(
        `${tui.commandInfo('SHOWCASE')} ${tui.info()} Showcase entry point created successfully at ${pc.inverse(showcaseEntryPointPath)}.`,
    )
    // Generate the showcase registry file
    generate_showcase_registry_file(frontoComponentsPath, projectRootPath)
}

function generate_showcase_registry_file(
